...
def register():
    ...
    if request.method == 'POST':
        ...
        if not organization:
            flash('Invalid organization selected.')
            return redirect(url_for('register'))
...
def dashboard():
    ...
    if current_user.organization is None:
        flash('No organization associated with the current user.', 'danger')
        return redirect(url_for('login'))
    if request.method == 'POST':
...
# Add this function to insert organizations if not present
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        insert_organizations()  # Ensure organizations are present
        create_or_update_admin_user()
    #app.run(debug=True)
    # Use Waitress to serve the application in production
